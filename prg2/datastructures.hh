...
public:
    ...
    // Estimate of performance: O(n)
    // Short rationale for estimate: this function used to be O(n^2) but now it is O(n) because we have a hash map that maps affiliation id to a vector of publications
    ...
    // Short rationale for estimate: this function used to be O(n^3) but now it is O(n^2) because we have a hash map that maps affiliation id to a vector of publications
...
private:
    ...
    std::unordered_map<AffiliationID, Affiliation> affiliationByIds;
    //std::vector<std::pair<Name, AffiliationID>> affiliationsByName;

    std::vector<AffiliationID> allAffiliationIDs;
    std::vector<Affiliation> allAffiliations;

    std::vector<Publication> allPublications;
    std::unordered_map<PublicationID, Publication> publicationByIds;
    std::unordered_map<AffiliationID, std::vector<PublicationID>> affiliationToPublications;

    std::vector<Connection*> allConnections;
    std::unordered_map<AffiliationID, std::vector<Connection*>> connectionById;

    void pretraversal(PublicationID id, std::vector<PublicationID>&result);
    ...
    bool findPath(AffiliationID current, AffiliationID target, std::vector<Connection>& path, std::unordered_map<AffiliationID, bool>& visited);
    // void create_connection_for_all();
    double calculateFriction(const Path& path);
    void findAllPaths(AffiliationID source, AffiliationID target, std::vector<Connection>& currentPath, std::vector<std::vector<Connection>>& allPaths, std::unordered_set<AffiliationID>& visited);
